...
new Vue({
    ...
    methods:
    {
        ...
        async getList()
        {
            ...
            await service.get(`/Admin/Transactions/Index?handler=Transactions&pageIndex=${pageIndex}&pageSize=${pageSize}&keyword=${keyword}&orderField=${orderField}`).then(res => {
            ...
            that.$refs['dataForm'].validate(valid => {
                ...
                if (valid)
                {
                    ...
                    let data = {
                        ...
                        Quota: that.dialog.data.quota,
                        Method: that.dialog.data.method
                    };
                    console.log('add-' + JSON.stringify(data));
                    service.post("/Admin/Transactions/Edit?handler=Save", data).then(res => {
                        if (res.data.success)
                        {
                            that.getList();
                            that.$notify({
        ...
        handleDelete(index, row) {
            let that = this
            let ids = [row.id];
            service.post("/Admin/Transactions/edit?handler=Delete", ids).then(res => {
                if (res.data.success)
                    {
                        that.getList();
                        that.$notify({
